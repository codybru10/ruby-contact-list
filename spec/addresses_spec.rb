...
describe(Address) do
  ...
  describe('.clear') do
    it('clears the address array') do
      Address.new({:address => "100 Infinity Loop", :city => "Gotham", :state => "New York",:zip => "80431"}).save()
      Address.clear()
      expect(Address.all()).to(eq([]))
    end
  end
  ...
  describe('.find') do
    it('finds an specific address based on id') do
      test_address = Address.new({:address => "100 Infinity Loop", :city => "Gotham", :state => "New York",:zip => "80431"})
      test_address.save()
      test_address2 = Address.new({:address => "512 CR 42", :city => "Helena", :state => "Ohio",:zip => "43435"})
      test_address2.save()
